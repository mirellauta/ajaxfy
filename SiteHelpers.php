...
class SiteHelpers
{
	...
	{
		$group_sql = " AND main_menu_access.group_id ='".Session::get('gid')."' ";
		$active 	=  ($active =='all' ? "" : "AND active ='1' ");
		$Q = DB::select("
		SELECT 
			main_menu.*
		FROM main_menu WHERE parent_id ='". $parent ."' ".$active." AND position ='{$position}'
		 ORDER BY ordering			
		");					
		return $Q;					
	}
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function formShow( $type , $field , $required ,$option = array()){
		...
		switch($type)
		{
			default;
				...
				if(isset($option['prefix']) && $option['prefix'] !='' or isset($option['sufix']) && $option['sufix'] !='')
				{
					...
					if($option['prefix'] !='')
						...
						$form .= "<input  type='text' name='{$field}' id='{$field}' value='@if (\$action=='modify'){{ \$row['{$field}'] }}}}@else  @endif' 
			...
			case 'maps';
				...
				<input  type='text' name='{$field}' id='{$field}' value='@if (\$action=='modify'){{ \$row['{$field}'] }}@else  @endif' 
			...
			case 'hidden';
				...
				$form = "@if (\$action=='modify'){!! Form::hidden('{$field}', \$row['{$field}']) !!} @endif";
				break;

			case 'textarea';
				
				$form = "<textarea name='{$field}' rows='5' id='{$field}' class='form-control form-control-sm {$extend_class}'  
				         {$mandatory} {$attribute} >@if (\$action=='modify'){{ \$row['{$field}'] }}@else  @endif</textarea>";
				break;
	...
	public static function toMasterDetail( $info )
	{
		...
		\$id = ''; 	
	 	...
	 	\$this->data['subform'] = \$this->detailview(\$this->modelview ,  \$this->info['config']['subform'] ,\$id );";
		 	...
		 	$tpl = array();
		 	require_once('../resources/views/main/module/template/native/masterdetailform.php');
	...
	public static function showMaps( $val ){
		...
		if(count($val) ==2) {
			...
			return '<iframe
			  ...
			  src="https://www.google.com/maps/embed/v1/place?key='.config("main.cnf_maps").'&q='.$lang.','.$long.'" allowfullscreen>
			</iframe>';		
	...
	public static function showFormMaps( $val , $id  ){
		...
		require_once('../resources/views/main/module/utility/maps.php');
	}
	...
	public static function createPreCombo( $field , $table , $key ,  $val ,$app ,$class ,$lookey = null)
	{
		...
		{ ".$parent." selected_value : '@if (\$action=='modify'){{ \$row[\"{$field}\"] }}@endif' });
		";	
		return $pre_jCombo;
	}	
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			...
			if($format_value =='')
			{
				if($mainconfig['cnf_date'] !='' )
				{
					$value = date("".$mainconfig['cnf_date']."",strtotime($value));
				} 
			} else {
	...
	{
		$path = base_path().'/resources/views/main/module/template/';
		$fld = scandir($path);
	...
	public static function avatar( $width =75 , $id = '')
	{
		...
		$Q = DB::table("main_users")->where("id",'=',$id)->get();
		if(count($Q)>=1) 
		{
			$row = $Q[0];
	...
	public static function BBCode2Html($text) {
		...
		$emotion =  url('main/js/plugins/markitup/images/emoticons/');
	...
	public static function auditTrail( $request , $note )
	{
		...
		\DB::table( 'main_logs')->insert($data);		
  ...
  static function storeNote( $args )
  {
      $args =  array_merge( array(
        ...
        \DB::table('main_notification')->insert($args);   
  }	 
  ...
  static function ResumeUserStatus()
  {
	...
	<!--	<div class="dropdown ">
		...
		if($access['is_detail'] ==1) {
			if($setting['view-method'] != 'expand')
			{
				...
				if($setting['view-method'] =='modal')
						$onclick = " onclick=\"MainModal(this.href,'View Detail'); return false; \"" ;
				$html .= '<!--<li class="nav-item">--><a class="tips btn-primary btn-sm" href="'.url($module.'/'.$id).'" '.$onclick.' class="nav-link " >
		...
		if($access['is_edit'] ==1) {
			...
			if($setting['form-method'] =='modal')
					$onclick = " onclick=\"MainModal(this.href,'Edit Form'); return false; \"" ;			
			
			$html .= '<!--<li class="nav-item">--><a class="tips btn-info btn-sm" href="'.url($module.'/'.$id).'/edit" '.$onclick.'  class="nav-link"><i class="far fa-edit"></i></a>
			<a  class="tips btn-danger btn-sm delete-single" data-id="'.$id.'" title="Delete"><i class="fas fa-trash-alt"></i></a>
	...
	static public function buttonActionCreate( $module  ,$method = 'newpage')
	{
		...
		if($method['form-method'] =='modal')
				$onclick = " onclick=\"MainModal(this.href,'Create Detail'); return false; \"" ;
	...
	static public function htmlExpandGrid()
	{
		...
		return View::make('main.module.utility.extendgrid');
	}
